...
<div class="flex h-full min-h-0 flex-col">
	...
	{#if source === 'docker' && sources && !sources.containers}
		<p class="px-4 py-3 text-sm text-amber-400">Add the SSH user to the docker group to read container logs.</p>
	{/if}
	{#if error}
	...
	<pre
		bind:this={viewport}
		class="min-h-0 flex-1 overflow-auto p-4 font-mono text-xs leading-relaxed whitespace-pre text-zinc-300">{shown ||
