<p align="center">
  <img src="static/icon-512.png" width="112" alt="Server icon" />
</p>
...
1. Create a key for your machine and authorize it on the server:
   ...
   ```sh
   ssh-keygen -t ed25519 -N '' -C server-dev -f ~/.ssh/server_dev
   ssh-copy-id -i ~/.ssh/server_dev.pub youruser@SERVER_IP
   ```
...
```sh
mkdir -p ~/server/secrets && cd ~/server
...
ssh-keygen -t ed25519 -N '' -C server -f secrets/ssh_key
...
The app listens on `127.0.0.1:3010` (set `SERVER_PORT` to change it). Put your authenticating proxy in front of it and set `ORIGIN` to the public URL, for example `https://server.example.com`.
...
- **"Could not reach the server over SSH":** check the output of `bun run dev` or `docker compose logs server`.
